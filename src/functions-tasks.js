...
function getPolynom(...args) {
  return function polynom(x) {
    const y = args.reduce((acc, currVal, currIndex) => {
      return acc + currVal * x ** (args.length - currIndex - 1);
    }, 0);
    return y;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return (...restArgsToApply) => {
    return fn(...args1, ...restArgsToApply);
  };
}
